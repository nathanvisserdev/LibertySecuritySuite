...
class ReqMonVM: ObservableObject {
    ...
    private func sendNotification(for request: TCCAccessRequest) {
        ...
        let preferences = MonitoringPreferences.shared
        
        if !preferences.shouldNotify(for: request) {
            return
        }
        
        // Check if app is trusted
        let bundleId = AppTrustManager.shared.getBundleId(from: request.processName)
        if AppTrustManager.shared.isAppTrusted(processName: request.processName, bundleId: bundleId) {
            return
        }
