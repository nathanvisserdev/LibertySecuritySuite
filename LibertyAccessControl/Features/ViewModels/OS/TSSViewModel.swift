...
struct TCCAccessRequest: Identifiable {
    let id = UUID()
    let timestamp: Date
...
class TSSViewModel: ObservableObject {
    @Published var isTSSEnabled: Bool = false
    @Published var statusMessage: String = "TCC monitoring not enabled"
